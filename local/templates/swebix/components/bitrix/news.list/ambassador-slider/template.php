...
?>
<div class="uk-ambassador uk-slideshow" data-uk-slideshow="ratio: false" data-uk-height-match="target: .uk-height">
	<ul class="uk-slideshow-items uk-grid uk-grid-collapse uk-height" data-uk-grid <?/*data-uk-height-match="target: > div > .uk-panel"*/?>>
		<?
			$i = 1;
		?>
		<?foreach($arResult["ITEMS"] as $arItem):?>
			<li class="uk-height uk-width-1-1">
				<?
				$this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
				$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
				?>
				<?$count = $i++;?>
				<div class="uk-panel uk-height<?if($count % 2 == 0):?> uk-right<?endif;?>" <?if($arItem['DISPLAY_PROPERTIES']['AMBASSADOR_BG']['DISPLAY_VALUE']):?>style="background-color: #<?=$arItem['DISPLAY_PROPERTIES']['AMBASSADOR_BG']['DISPLAY_VALUE']?>; color: #<?=$arItem['DISPLAY_PROPERTIES']['AMBASSADOR_COLOR']['DISPLAY_VALUE']?>"<?endif;?> <?if($arItem["PROPERTIES"]["AMBASSADOR_IMAGE"]["VALUE"]):?>data-src="<?=CFile::GetPath($arItem["PROPERTIES"]["AMBASSADOR_IMAGE"]["VALUE"])?>" data-uk-img<?endif;?>>
	...
	<div class="uk-dotnav-panel uk-flex uk-flex-center">
		<div class="uk-grid uk-grid-small uk-flex uk-flex-middle uk-width-1-1" data-uk-grid>
			<div class="uk-width-auto">
				<a class="uk-slidenav" href="#"uk-slideshow-item="previous">
					<span data-uk-icon="icon: arrow-left; ratio: 1"></span>
				</a>
			</div>
			...
			<div class="uk-width-auto">
				<a class="uk-slidenav" href="#" uk-slideshow-item="next">
					<span data-uk-icon="icon: arrow-right; ratio: 1"></span>
				</a>
			</div>
		</div>
